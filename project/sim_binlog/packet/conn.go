...
func (c *Conn) ReadPacketTo(w io.Writer) error {
	...
	if sequence != c.Sequence {
		return errors.New("invalid sequence %d != %d")
	}
	...
	if n, err := c.copyN(w, c.reader, int64(length)); err != nil {
		return errors.New("io.CopyN failed. err %v, copied %v, expected %v")
	} else if n != int64(length) {
		return errors.New("io.CopyN failed(n != int64(length)). %v bytes copied, while %v expected")
	} else {
		if length < MaxPayloadLen {
			return nil
		}
		if err := c.ReadPacketTo(w); err != nil {
			return errors.New("ReadPacketTo failed")
		}
	}
...
func (c *Conn) WritePacket(data []byte) error {
	...
	if n, err := c.Write(data); err != nil {
		...
		return errors.New("Write failed. only %v bytes written, while %v expected")
	} else {
		c.Sequence++
		return nil
	}
...
func (c *Conn) WriteClearAuthPacket(password string) error {
	...
	c.WritePacket(data)
	return errors.New("WritePacket failed")
}
...
func (c *Conn) WriteEncryptedPassword(password string, seed []byte, pub *rsa.PublicKey) error {
	...
	c.WriteAuthSwitchPacket(enc, false)
	return errors.New("WriteAuthSwitchPacket failed")
}
