...
func FormatBinaryDate(n int, data []byte) ([]byte, error) {
	...
	case 4:
		return []byte(fmt.Sprintf("%04d-%02d-%02d",
			...
			data[3])), nil
	default:
		return nil, errors.New("invalid date packet length %d")
	}
}
...
func FormatBinaryDateTime(n int, data []byte) ([]byte, error) {
	...
	default:
		return nil, errors.New("invalid datetime packet length %d")
	}
}
...
func FormatBinaryTime(n int, data []byte) ([]byte, error) {
	...
	default:
		return nil, errors.New("invalid time packet length %d")
	}
}
